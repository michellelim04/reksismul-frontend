	...
	const getDataDetailAssignment = async (id: string) => {
		try {
			...
			const detailAssigment = await fetch(
				`http://localhost:3333/v1/submission/list?id=${id}`,
				{
					headers: {
						Authorization: `${token}`,
					},
			...
			<>
				<section className="bg-white p-5">
					<div className="flex mb-5 flex-col">
						...
						<span className="mt-2">
							Deadline : {moment(dateDeadline).format("DD/MM/YYYY")}{" "}
							{timeDeadline}
						</span>
					</div>
					<div className="hidden md:flex">
						<table className="table table-fixed w-full md:table">
							<thead
								className={`${poppinsB.className} text-white bg-[#0E8388]`}
							>
								<tr>
							...
							<tbody className={poppins.className}>
								{listAssigment.length <= 0 ? (
									...
									listAssigment.map((e: any, i) => {
										return (
											<tr
												className={i % 2 === 0 ? "bg-white" : "bg-[#CBE4DE]"}
												key={i}
											>
												...
												<td className="p-3 text-center align-middle">
													{e.submission !== undefined
														? moment(e.submission.created_at).format("HH.mm")
														: "-"}
												</td>
												<td className="p-3 text-center align-middle">
													{e.submission !== undefined
